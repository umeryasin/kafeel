...
<html lang="en">
  <head>
    ...
    <title>View Companies - Kafeel</title>
  ...
  <body class="az-body az-body-sidebar">
    ...
    <?php include_once('nav/second_header.php'); ?>
      ...
      <div class="az-content-header d-block d-md-flex">
        <div>
          <h2 class="az-content-title mg-b-5 mg-b-lg-8">View Companies</h2>
          <p class="mg-b-0">Active Companies</p>
        </div>
      ...
      <div class="az-content-body">
        <div class="table-responsive">
          <table class="table table-hover table-bordered table-striped">
            ...
            <tbody>
              <?php
              $sql_all="SELECT * FROM `wakeel_info` WHERE `wi_sub_admin_id`=$admin_id AND `wi_del_status`=0";
              ...
              while($row_all=mysqli_fetch_assoc($result_all))
              {
                ...
                <tr>
                  ...
                  <td><?php echo $row_all['wi_starting_date']; ?></td>
                  <td><?php echo $row_all['wi_expire_date']; ?></td>
                  <td>
                    <?php
                    if($row_all['wi_act_status']==1)
                    {
                      echo "Active";
                    }
                    else
                  ...
                  <td>
                     <input type="checkbox" name="change_status_input" id="change_status_input" class="change_status_input" <?php
                    if($row_all['wi_act_status']==1)
                    {
                      echo "checked";
                    }
                    ...
                    ?> data-toggle="toggle" data-on="Deactive" data-off="Active" value="<?php echo $row_all['wi_id']; ?>">
                  </td>
                  <td>
                    <button class="btn btn-info view-btn" value="<?php echo $row_all['wi_id']; ?>"><i class="far fa-eye"></i></button>
                    <button class="btn btn-primary edit-btn" data-toggle="modal" data-target="#modaldemo1" value="<?php echo $row_all['wi_id']; ?>"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-danger del-btn" value="<?php echo $row_all['wi_id']; ?>"><i class="fas fa-trash-alt"></i></button>
                  </td>
                </tr>
                <?php
              }
  ...
  <div id="modaldemo1" class="modal">
    ...
    <script type="text/javascript">
      ...
      $("#view-adv-active").addClass("active");
    </script>
    <script type="text/javascript">
    ...
    <script type="text/javascript">
      $(".change_status_input").on("change",function(){
        var status_id=$(this).val();
        if($(this).prop("checked") == true)
            {
                //alert("Checkbox is checked.");
                //To active the advocate
                $.ajax({
                  method:'POST',
                  url:'module/advocate/ajax/change_status_adv_ajax.php',
                  data:{status_id_act:status_id},
                  success: function(data)
                  {
                    console.log(data);
                    if(data==true)
        ...
        else if($(this).prop("checked") == false)
            {
                ...
                //To Deactive the advocate
                $.ajax({
                  method:'POST',
                  url:'module/advocate/ajax/change_status_adv_ajax.php',
                  data:{status_id_deact:status_id},
                  success: function(data)
                  {
                    console.log(data);
                    if(data==true)
